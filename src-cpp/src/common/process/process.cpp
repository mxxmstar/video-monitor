...
boost::system::error_code MakeError(boost::system::errc::errc_t errc) {
...
public:
    explicit ProcessImpl(boost::asio::any_io_executor executor)
    ...
    void AsyncWait(Process::ExitHandler handler) {
        ...
        proc->async_wait(
            [proc, handler = std::move(handler)](const boost::system::error_code& ec, int exit_code) mutable {
...
private:
    /// @brief 设置进程错误信息
    bool SetError(boost::system::error_code source, std::string message, boost::system::error_code& out) noexcept {
        out = source;
    ...
    std::shared_ptr<bpv2::process> LaunchWithInits(const ProcessOptions& options,
        ...
        if (options.working_dir.empty()) {
            return std::make_shared<bpv2::process>(
                executor_, executable, options.args, std::forward<Inits>(inits)...);
        }
        ...
        return std::make_shared<bpv2::process>(
            ...
            executable,
            options.args,
            bpv2::process_start_dir{ToBoostPath(options.working_dir)},
...
Process::Process(boost::asio::any_io_executor executor)
    : impl_(std::make_unique<ProcessImpl>(std::move(executor))) {
}
...
bool Process::Start(const ProcessOptions& options, boost::system::error_code& ec) noexcept {
...
bool Process::IsRunning(boost::system::error_code& ec) noexcept {
...
bool Process::RequestExit(boost::system::error_code& ec) noexcept {
    return impl_->RequestExit(ec);
}

bool Process::Terminate(boost::system::error_code& ec) noexcept {
...
int Process::Wait(boost::system::error_code& ec) noexcept {
